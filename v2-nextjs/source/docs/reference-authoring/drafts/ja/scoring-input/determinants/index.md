...
displayTitle: "[コーディング/決定因] Determinants"
aliases:
  - "Determinants"
  - "決定因"
  - "運動"
...
status: "draft"
runtimeReady: false
provenanceNote: "docs/reference-authoring/notes/provenance/ja/scoring-input/determinants/index.md"
---

# Document Name: [コーディング/決定因] Determinants
...
Determinants は、その反応が「何によってそう見えたか」をコード化する軸です。
何が見えたかではなく、形・運動・色・陰影・反射・奥行きなど、どのブロット特性が反応を支えたかを記録します。

## Application Conditions

- 判定は受検者の言葉と inquiry に基づき、検査者の自由推測だけで決めない。
- 一つの反応に複数の determinant が実際に含まれるなら複合的にコード化できる。
- 複合反応では、反応段階で現れた順序に従って determinant を並べる。
- 現在の日本語 authoring では、まず運動 [`M`](ref://scoring-input/determinants/M), [`FM`](ref://scoring-input/determinants/FM), [`m`](ref://scoring-input/determinants/m) と、色彩 [`FC`](ref://scoring-input/determinants/FC), [`CF`](ref://scoring-input/determinants/CF), [`C`](ref://scoring-input/determinants/C) を中心に開いている。

## Cautions / Distinctions

- Determinants は [`FQ`](ref://scoring-input/fq) や Location の代わりではありません。別の軸です。
- 運動では「人間的か、動物的か、無生物的か」を区別します。
- 色彩では「形が主か、色が主か」を区別して `FC`, `CF`, `C` を分けます。
- 後続の achromatic color, texture, vista, diffuse shading もすべて同じ陰影群としてまとめず、別々に扱います。
- 解釈的意味はあとで [`FC:CF+C`](ref://result-interpretation/lower-section/affect/FC_CF_C), [`Pure C`](ref://result-interpretation/lower-section/affect/PureC), [`Lambda`](ref://result-interpretation/lower-section/core/Lambda), [`FM`](ref://result-interpretation/lower-section/core/FM), [`m`](ref://result-interpretation/lower-section/core/m) などで検討します。
