---
canonicalRoute: "scoring-input/determinants/VF"
locale: "ja"
docKind: "coding-entry"
canonicalTitle: "scoring-input/determinants/VF"
displayTitle: "[コーディング/決定因] VF"
aliases:
  - "VF"
  - "Vista-形態"
  - "奥行き優位"
relatedRoutes:
  - "scoring-input/determinants"
  - "scoring-input/determinants/FV"
  - "scoring-input/determinants/V"
  - "scoring-input/determinants/F"
  - "scoring-input/determinants/Y"
  - "result-interpretation/lower-section/core/SumV"
authorityPolicy: "exner-base"
status: "draft"
runtimeReady: false
provenanceNote: "docs/reference-authoring/notes/provenance/ja/scoring-input/determinants/vf/index.md"
---

# Document Name: [コーディング/決定因] VF

## Aliases / Search Terms

- VF
- Vista-形態
- 奥行き優位

## Core Definition

`VF` は、奥行き・距離・立体感が反応を主に支え、形は補助的に残っているときにつける determinant です。
形はあるが、反応を押しているのは深さや遠近感の方です。

## Application Conditions

- Vista が反応の主要な基盤になっている。
- 形の支えもあるが、明らかに二次的である。
- 形が主なら [`FV`](ref://scoring-input/determinants/FV) を再検討する。
- 形の支えがほとんどないなら [`V`](ref://scoring-input/determinants/V) を考える。

## Cautions / Distinctions

- `VF` は `FV` より Vista の重みが強い反応です。
- 質感が中心なら [`TF`](ref://scoring-input/determinants/TF) や [`T`](ref://scoring-input/determinants/T) を見直します。
- 拡散的陰影が中心なら [`Y`](ref://scoring-input/determinants/Y) を見直します。

## Cross References

- [[コーディング/決定因] Determinants](ref://scoring-input/determinants)
- [[コーディング/決定因] FV](ref://scoring-input/determinants/FV)
- [[コーディング/決定因] V](ref://scoring-input/determinants/V)
- [[コーディング/決定因] F](ref://scoring-input/determinants/F)
- [[コーディング/決定因] Y](ref://scoring-input/determinants/Y)
- [[解釈/Core] SumV](ref://result-interpretation/lower-section/core/SumV)

## Evidence Note

- 詳細な出典比較と判断根拠は内部 provenance note に分離してあります。
