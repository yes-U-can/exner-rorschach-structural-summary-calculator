---
canonicalRoute: "scoring-input/contents"
locale: "ja"
docKind: "coding-overview"
canonicalTitle: "scoring-input/contents"
displayTitle: "[採点/内容] Contents"
aliases:
  - "Contents"
  - "内容コード"
  - "反応内容"
  - "内容カテゴリー"
relatedRoutes:
  - "scoring-input/contents/H"
  - "scoring-input/contents/(H)"
  - "scoring-input/contents/Hd"
  - "scoring-input/contents/(Hd)"
  - "scoring-input/contents/Hx"
  - "scoring-input/contents/A"
  - "scoring-input/contents/(A)"
  - "scoring-input/contents/Ad"
  - "scoring-input/contents/(Ad)"
  - "scoring-input/contents/An"
  - "scoring-input/contents/Art"
  - "scoring-input/contents/Ay"
  - "scoring-input/contents/Bl"
  - "scoring-input/contents/Bt"
  - "scoring-input/contents/Cg"
  - "scoring-input/contents/Cl"
  - "scoring-input/contents/Ex"
  - "scoring-input/contents/Fi"
  - "scoring-input/contents/Fd"
  - "scoring-input/contents/Ge"
  - "scoring-input/contents/Hh"
  - "scoring-input/contents/Ls"
  - "scoring-input/contents/Na"
  - "scoring-input/contents/Sc"
  - "scoring-input/contents/Sx"
  - "scoring-input/contents/Xy"
  - "scoring-input/contents/Id"
  - "scoring-input/gphr/GHR"
  - "scoring-input/gphr/PHR"
  - "result-interpretation/lower-section/interpersonal/HumanCont"
  - "result-interpretation/lower-section/interpersonal/PureH"
  - "result-interpretation/lower-section/interpersonal/Food"
  - "result-interpretation/lower-section/selfPerception/An_Xy"
  - "result-interpretation/lower-section/selfPerception/H_ratio"
authorityPolicy: "exner-base"
status: "draft"
runtimeReady: false
provenanceNote: "docs/reference-authoring/notes/provenance-ja-contents-ls-na-sc-sx-xy-id-2026-03-10.md"
---

# Document Name: [採点/内容] Contents

## Aliases / Search Terms

- Contents
- 内容コード
- 反応内容
- 内容カテゴリー

## Core Definition

`Contents`は、被検者が何を見たかを記録する内容コード群です。  
決定因やFQ、特殊スコアとは別の軸で、反応の主題や対象の種類を整理します。

## Application Conditions

- 内容コードは、反応が何として把握されたかを記録するために使います。
- 人間内容では[`H`](ref://scoring-input/contents/H)、[`(H)`](ref://scoring-input/contents/%28H%29)、[`Hd`](ref://scoring-input/contents/Hd)、[`(Hd)`](ref://scoring-input/contents/%28Hd%29)、[`Hx`](ref://scoring-input/contents/Hx)を区別します。
- 動物内容では[`A`](ref://scoring-input/contents/A)、[`(A)`](ref://scoring-input/contents/%28A%29)、[`Ad`](ref://scoring-input/contents/Ad)、[`(Ad)`](ref://scoring-input/contents/%28Ad%29)、[`An`](ref://scoring-input/contents/An)を使い分けます。
- 芸術・文化・自然寄りの内容では[`Art`](ref://scoring-input/contents/Art)、[`Ay`](ref://scoring-input/contents/Ay)、[`Bl`](ref://scoring-input/contents/Bl)、[`Bt`](ref://scoring-input/contents/Bt)を検討します。
- 衣類・雲・爆発・火や煙では[`Cg`](ref://scoring-input/contents/Cg)、[`Cl`](ref://scoring-input/contents/Cl)、[`Ex`](ref://scoring-input/contents/Ex)、[`Fi`](ref://scoring-input/contents/Fi)を使います。
- 食べ物・地図・家庭用品では[`Fd`](ref://scoring-input/contents/Fd)、[`Ge`](ref://scoring-input/contents/Ge)、[`Hh`](ref://scoring-input/contents/Hh)を検討します。
- 風景・自然・科学技術・性的内容・X線・特異内容では[`Ls`](ref://scoring-input/contents/Ls)、[`Na`](ref://scoring-input/contents/Na)、[`Sc`](ref://scoring-input/contents/Sc)、[`Sx`](ref://scoring-input/contents/Sx)、[`Xy`](ref://scoring-input/contents/Xy)、[`Id`](ref://scoring-input/contents/Id)を検討します。

## Cautions / Distinctions

- 内容コードは、決定因やFQの代わりではありません。
- `H`と`(H)`、`Hd`と`(Hd)`、`A`と`(A)`、`Ad`と`(Ad)`は、現実性と部分性を丁寧に分けて考える必要があります。
- [`Art`](ref://scoring-input/contents/Art)と[`Ay`](ref://scoring-input/contents/Ay)は近いですが、前者は芸術作品、後者は神話的・空想的存在に重心があります。
- [`Fd`](ref://scoring-input/contents/Fd)と[`FD`](ref://scoring-input/determinants/FD)は似た記号ですが、前者は内容、後者は決定因です。
- [`Ge`](ref://scoring-input/contents/Ge)は地図であり、風景なら[`Ls`](ref://scoring-input/contents/Ls)、自然環境なら[`Na`](ref://scoring-input/contents/Na)を優先して考えます。
- [`Sc`](ref://scoring-input/contents/Sc)は科学技術的な読みが必要で、単に変わった内容というだけでは`Id`にしません。
- [`Sx`](ref://scoring-input/contents/Sx)は明確な性的意味が必要で、単なる身体形態だけでは自動的に付けません。
- [`Xy`](ref://scoring-input/contents/Xy)はX線的形式を示し、同じ反応に[`An`](ref://scoring-input/contents/An)を併記しない点が重要です。

## Cross References

- [[採点/内容] H](ref://scoring-input/contents/H)
- [[採点/内容] (H)](ref://scoring-input/contents/%28H%29)
- [[採点/内容] Hd](ref://scoring-input/contents/Hd)
- [[採点/内容] (Hd)](ref://scoring-input/contents/%28Hd%29)
- [[採点/内容] Hx](ref://scoring-input/contents/Hx)
- [[採点/内容] A](ref://scoring-input/contents/A)
- [[採点/内容] (A)](ref://scoring-input/contents/%28A%29)
- [[採点/内容] Ad](ref://scoring-input/contents/Ad)
- [[採点/内容] (Ad)](ref://scoring-input/contents/%28Ad%29)
- [[採点/内容] An](ref://scoring-input/contents/An)
- [[採点/内容] Art](ref://scoring-input/contents/Art)
- [[採点/内容] Ay](ref://scoring-input/contents/Ay)
- [[採点/内容] Bl](ref://scoring-input/contents/Bl)
- [[採点/内容] Bt](ref://scoring-input/contents/Bt)
- [[採点/内容] Cg](ref://scoring-input/contents/Cg)
- [[採点/内容] Cl](ref://scoring-input/contents/Cl)
- [[採点/内容] Ex](ref://scoring-input/contents/Ex)
- [[採点/内容] Fi](ref://scoring-input/contents/Fi)
- [[採点/内容] Fd](ref://scoring-input/contents/Fd)
- [[採点/内容] Ge](ref://scoring-input/contents/Ge)
- [[採点/内容] Hh](ref://scoring-input/contents/Hh)
- [[採点/内容] Ls](ref://scoring-input/contents/Ls)
- [[採点/内容] Na](ref://scoring-input/contents/Na)
- [[採点/内容] Sc](ref://scoring-input/contents/Sc)
- [[採点/内容] Sx](ref://scoring-input/contents/Sx)
- [[採点/内容] Xy](ref://scoring-input/contents/Xy)
- [[採点/内容] Id](ref://scoring-input/contents/Id)
- [[採点/GHR-PHR] GHR](ref://scoring-input/gphr/GHR)
- [[採点/GHR-PHR] PHR](ref://scoring-input/gphr/PHR)
- [[解釈/対人関係] Human Content](ref://result-interpretation/lower-section/interpersonal/HumanCont)
- [[解釈/対人関係] Pure H](ref://result-interpretation/lower-section/interpersonal/PureH)
- [[解釈/対人関係] Food](ref://result-interpretation/lower-section/interpersonal/Food)
- [[解釈/自己知覚] An + Xy](ref://result-interpretation/lower-section/selfPerception/An_Xy)
- [[解釈/自己知覚] H ratio](ref://result-interpretation/lower-section/selfPerception/H_ratio)

## Evidence Note

- 詳細な出典比較は内部 provenance note に記録しています。
