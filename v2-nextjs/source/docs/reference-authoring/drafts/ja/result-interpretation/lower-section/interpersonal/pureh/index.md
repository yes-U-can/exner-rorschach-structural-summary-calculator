---
canonicalRoute: "result-interpretation/lower-section/interpersonal/PureH"
locale: "ja"
docKind: "interpretation-entry"
canonicalTitle: "result-interpretation/lower-section/interpersonal/PureH"
displayTitle: "[Interpretation/Interpersonal] Pure H"
aliases:
  - "Pure H"
  - "pure human"
  - "whole human content"
  - "完全な人間表象"
relatedRoutes:
  - "result-interpretation/lower-section/interpersonal"
  - "result-interpretation/lower-section/interpersonal/HumanCont"
  - "scoring-input/contents/H"
  - "scoring-input/contents/%28H%29"
  - "scoring-input/gphr/GHR"
  - "scoring-input/gphr/PHR"
  - "result-interpretation/lower-section/selfPerception/H_ratio"
  - "result-interpretation/special-indices/CDI"
authorityPolicy: "exner-base"
status: "draft"
runtimeReady: false
provenanceNote: "docs/reference-authoring/notes/provenance-ja-interpersonal-pureh-per-iso-2026-03-10.md"
---

# Document Name: [Interpretation/Interpersonal] Pure H

## Aliases / Search Terms

- Pure H
- pure human
- whole human content
- 完全な人間表象

## Core Definition

`PureH` は、人間内容が完全で通常の形、つまり `H` としてどれくらい現れるかを示す。
対人的水準では、相手のイメージが part-object や partial figure、fantasy human ではなく、全体的な人物として組織化されやすいかをみる助けになる。

## Interpretation Points

- `PureH` が比較的高いと、人をより全体的で統合された像として表しやすいことが多い。
- `HumanCont` は低くないのに `PureH` が低いなら、人間表象がより部分的、理想化的、あるいは歪みを帯びていないかを確認したい。
- `PureH < 2` のときは、対人スタイルと human content の質をより慎重に見直す根拠になりやすい。
- この変数は human representation の structural quality を示すのであって、親切さや共感性そのものではない。

## Cautions / Distinctions

- `PureH` と `HumanCont` は同じではない。前者は complete human content、後者は total human content を見る。
- 高くても `PHR`、`AG`、`MOR` が高いなら、成熟した relatedness を保証しない。
- 低いからといって、それだけで人への拒否を意味するわけではない。

## Limits of Isolated Interpretation

- `PureH` は `HumanCont`、`GHR`、`PHR`、`H ratio` と比べて読む必要がある。
- human response 自体が非常に少ない記録では意味づけが変わる。

## Variables to Review Together

- `HumanCont`
- `GHR`
- `PHR`
- `H ratio`
- `CDI`

## Cross References

- [[Interpretation/Lower Section/Interpersonal] Interpersonal](ref://result-interpretation/lower-section/interpersonal)
- [[Interpretation/Interpersonal] Human Content](ref://result-interpretation/lower-section/interpersonal/HumanCont)
- [[Coding/Content] H](ref://scoring-input/contents/H)
- [[Coding/Content] (H)](ref://scoring-input/contents/%28H%29)
- [[Coding/GHR-PHR] GHR](ref://scoring-input/gphr/GHR)
- [[Coding/GHR-PHR] PHR](ref://scoring-input/gphr/PHR)
- [[Interpretation/Self Perception] H Ratio](ref://result-interpretation/lower-section/selfPerception/H_ratio)
- [[Interpretation/Special Indices] CDI](ref://result-interpretation/special-indices/CDI)

## Evidence Note

- Detailed source comparison is stored in internal provenance notes.
