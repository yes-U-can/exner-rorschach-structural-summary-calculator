...
3. **같은 운동군 중복 차단**: `Ma/Mp/Ma-p`, `FMa/FMp/FMa-p`, `ma/mp/ma-p` 각 군에서 반응당 하나만 선택할 수 있다. UI에서 같은 군의 다른 코드는 비활성화되고, 계산기는 비정상 payload도 거부한다.
...
- 같은 반응에 동일한 비운동 결정인을 두 번 넣을 수 있는 기존 입력 허용은 blend 중복 금지 규칙(2차 인용)과 충돌할 수 있으므로, 제품 소유자 승인 후 같은 방식의 입력 경계로 다룰 후보로 남긴다.
