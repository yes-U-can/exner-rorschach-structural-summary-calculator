...
export const resultVariableDescriptions: Record<string, VariableDescription> = {
  ...
  FC_CF_C: localize(
    ...
    [
      ...
      'Operational basis: The conventional label remains FC:CF+C, while the displayed right side is calculated as CF+C+Cn. Cn remains excluded from WSumC and from S-CON criterion 7.',
...
const KO_DESCRIPTION_OVERRIDES: Partial<Record<string, string>> = {
  ...
  FC_CF_C: [
    ...
    'Operational basis: 관례적 이름은 FC:CF+C이지만 화면의 오른쪽 값은 CF+C+Cn으로 계산합니다. Cn은 WSumC와 S-CON 7번 기준에서는 제외합니다.',
...
const ES_DESCRIPTION_OVERRIDES: Partial<Record<string, string>> = {
  ...
  FC_CF_C: ['Definition: Razon de modulacion de color FC frente a CF+C.', 'Operational basis: La etiqueta convencional se mantiene, pero el lado derecho mostrado es CF+C+Cn. Cn se excluye de WSumC y del criterio 7 de S-CON.', 'Interpretive direction: FC dominante sugiere mayor modulacion; CF/C dominante menor control.', 'Cross-checks: Integrar con Pure C, Afr y D/AdjD.', 'Caution: Interpretar con base total de respuestas de color.'].join('\n'),
...
const PT_DESCRIPTION_OVERRIDES: Partial<Record<string, string>> = {
  ...
  FC_CF_C: ['Definition: Razao de modulacao de cor FC versus CF+C.', 'Operational basis: O rotulo convencional permanece, mas o lado direito exibido e CF+C+Cn. Cn e excluido de WSumC e do criterio 7 de S-CON.', 'Interpretive direction: FC dominante sugere maior modulacao; CF/C dominante menor controle.', 'Cross-checks: Integrar com Pure C, Afr e D/AdjD.', 'Caution: Interpretar com base total de respostas de cor.'].join('\n'),
...
const JA_DESCRIPTION_OVERRIDES: Partial<Record<string, string>> = {
  ...
  FC_CF_C: ['Definition: 色彩調整比（FC対CF+C）です。', 'Operational basis: 慣例的名称は維持しますが、表示上の右側はCF+C+Cnで算出します。CnはWSumCとS-CON基準7から除外します。', 'Interpretive direction: FC優位は調整性増加、CF/C優位は調整低下を示唆します。', 'Cross-checks: Pure C、Afr、D/AdjDと統合します。', 'Caution: 色彩反応総数を考慮して解釈します。'].join('\n'),
