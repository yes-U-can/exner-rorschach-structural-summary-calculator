...
> **2026-07-17 추가 확인:** v2.2.1에서 화면의 `FC:CF+C` 오른쪽 값에 Cn을 포함한 계산은 그대로 유지됩니다. 이 값은 실제 배포된 v2.2.1에서 이미 올바르게 계산됐으므로, 완성된 프로토콜을 이 이유만으로 다시 계산할 필요는 없습니다. v2.2.2에서는 이 화면값과 WSumC, S-CON 7번 기준, Color-Shading의 서로 다른 Cn 범위를 다시 확인했고, 형태질(FQ)이 비어 있는 미완성 행의 GHR/PHR 임시 분류도 보완했습니다. 최신 설명은 [v2.2.2 계산 정확성 핫픽스](../v2.2.2/)에서 확인할 수 있습니다.
...
v2.2.1은 v2.2.0 검산에서 남아 있던 오류를 바로잡고, 구조요약 계산식을 교정한 핫픽스입니다. 이번 릴리즈의 중심은 API 키 없이도 사용하는 **구조요약 계산기 자체의 정확성**이며, 검산 범위는 앱이 표시하는 **Upper Section, Lower Section, Special Indices**입니다.

위 세 영역 밖의 연구용·확장 지표나 다른 로샤 체계는 이번 검산에 포함하지 않았습니다. UI/UX는 변경하지 않았고 연령 입력란도 추가하지 않았습니다. 계산기는 지금까지처럼 피검자의 연령을 요구하거나 저장하지 않습니다. 부호화와 임상 해석은 전문가의 몫이며, 선택 기능인 AI 해석 도우미만 연령 제한이 있는 지표의 해석을 직접 요청받았을 때 대화 안에서 필요한 정보를 질문합니다.

## 먼저 바로잡는 v2.2.0 기록

v2.2.0 문서에는 계산 검산 결과가 함께 기록되어 있지만 일부 항목은 잘못 설명되어 있었습니다. 해당 기록은 삭제하지 않고 현재 계산 기준을 확인할 수 있도록 정정 안내를 추가했습니다.
...
- v1과 v2.2.0은 화면의 `FC:CF+C` 오른쪽 값에서 Cn을 누락했습니다. v2.2.1은 Cn을 포함하도록 교정했고, 실제 배포된 계산도 이 기준을 사용했습니다.
...
1. 초기 개발 과정에서는 프로그래밍을 배우기 전 Excel 수식으로 구조요약 계산을 이해했습니다.
2. v1 GAS를 만들며 해당 수식을 JavaScript로 옮겼습니다.
3. v2 Next.js에서 계산 로직이 TypeScript로 이어졌습니다.
4. 이번 검산에서는 Excel을 정답으로 가정하지 않고 Exner 명시 규칙, 완성 예제, CHESSSS, 공개 점수열과 나란히 대조했습니다.
...
| 자기중심성 지수 | 반사반응 x3 | x2로 잘못 옮김 | v2.2.0 이전에 x3 교정 | x3 유지·반복 확인 |
| all-F Lambda | 별도 문구 | 0으로 대체 | v2.2.0 이전에 무한대로 교정 | 무한대 유지·반복 확인 |
...
- **v1·v2.2.0의 Cn 화면 누락:** Cn이 한 개 이상인 비교적 좁은 범위에서 Lower Section 비율 오른쪽 값이 작게 표시됐습니다. 실제 배포된 v2.2.1은 이 값을 포함하도록 교정됐습니다.
...
6. 경계값 예시, 같은 오류가 다시 생기지 않는지 보는 반복 검사, 언어 불변성 검사, 같은 조건으로 반복 가능한 가상 프로토콜 검사
...
다섯 점수열 각각에 다섯 언어 메모를 넣어 총 25회 계산하고, 메모를 제외한 전체 계산 결과가 완전히 같은지 확인했습니다. Cn을 추가하면 Lower Section의 화면 비율만 Cn만큼 변하고 WSumC·S-CON·Color-Shading은 변하지 않는다는 점은 v2.2.2에서 다시 확인했습니다. 이것은 **언어 불변성 검사**, 즉 메모 언어가 계산값을 바꾸지 않는지 보는 검사입니다. 다섯 문화권의 임상 규준 타당성을 증명했다는 뜻은 아닙니다.
...
- 계산기에 연령을 요구하지 않고, 연령 제한이 있는 해석을 AI에 직접 요청한 경우에만 대화에서 확인하도록 답변 기준과 자동 검사에 고정했습니다.

## 어떻게 확인했나요?

- 전체 계산·기능 자동 검사: `68`개 파일, `356`개 통과, 의도적으로 제외한 검사 `6`개
- Exner Chapter 9의 17반응 완성 예제를 Upper Section, Lower Section, 여섯 Special Indices까지 처음부터 끝까지 대조
- 공개 점수열 3건과 2019 Excel 내장 사례 1건 반복 검사
- D/AdjD 경계와 상한 밖 값, EBPer 표시 조건, `FC:CF+C`의 Cn 포함, 0분모, GHR/PHR 7단계 집중 검사
- 5개 고정 예시 x 5개 언어 메모 = 25회 언어 불변성 검사
- 같은 조건으로 반복 가능한 가상 프로토콜 2,000개를 별도로 작성한 계산식과 대조
- PTI·DEPI·CDI·S-CON·HVI·OBS의 42개 분기에서 true와 false 양쪽 모두 통과
- 실제 AI 대화 경로 4건: 정상 응답, `gpt-5.5`, 중단 0, 발견된 문제 0
- 5개 언어 실제 GPT-5.5 호출 5건: 관련 참조 문서 검색 성공, 중단 0, 발견된 문제 0
- v2.2.1 당시에는 Cn의 네 계산 경계를 직접 묻는 별도 검사가 없었습니다. v2.2.2에서 이 검사를 추가해 영어 코딩·해석 6/6과 5개 언어 직접 질문 10/10을 통과했습니다.
- AI 검색 자료 6,632개 모두 최신 상태, 이전 내용이 남은 자료 0개
- 다섯 언어 참조 문서 질문: 한국어 101개, 스페인어 81개, 영어·일본어·포르투갈어 각 61개 전부 통과
- AI 답변 기준 91/91, 릴리즈 전 종합 점검 7/7, 배포용 페이지 생성 222/222, 알려진 취약점 0
- 실제 API 호출 기록에는 API 키, 질문 원문, 답변 원문, 구조요약 원문을 저장하지 않음
...
공개 소스에는 교정된 계산 코드, 반복·가상 프로토콜·언어 불변성 검사, 다섯 언어 계산 설명, 계산 추적 기록, 질문과 답변 원문을 제외한 실제 API 호출 요약을 포함합니다.

Excel 원본, Exner 원문 자료, API 키, `.env*`, Neon/Vercel 연결 정보, 질문·답변 원문, 대화 내용, 반응 메모, 구조요약 원문, 환자 정보, 로컬 임시 파일과 실행 기록은 공개하지 않습니다.
...
- 실제 GPT-5.5 호출 검사는 도우미가 정해 둔 설명 기준을 지키는지 확인한 것이며 임상적 인증이 아닙니다.
