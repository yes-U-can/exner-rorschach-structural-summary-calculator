...
- In **[Determinants]**, more than one code from the same movement family could be entered for a single response (for example, `Ma` and `Mp`). Once a code from a family is selected, the other codes in the same family can no longer be chosen.
...
- In the full automated suite, 83 test files passed all 471 checks; 7 checks were skipped because their run conditions were unavailable. The production build, static code checks, five-language copy audit, and secret scanning also passed.
