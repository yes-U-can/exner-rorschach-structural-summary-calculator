...
> **2026-07-17 정정:** v2.2.0의 UI/UX 변경 기록은 그대로 유효하지만 계산 감사의 일부 판정은 불완전했습니다. D/AdjD 상한, EBPer gate, GHR/PHR 순서, Lower Section `FC:CF+C`의 Cn 누락, 정의 불가 분모 처리는 v2.2.1에서 교정됐습니다. [v2.2.2 계산 정확성 재감사 핫픽스](../v2.2.2/)는 이 교정 결과를 다시 확인하고 Cn 관련 계산·문서·AI 평가 경계를 서로 분리해 고정했습니다. 계산기는 연령을 입력받지 않으며, 연령 제한이 있는 해석을 요청한 경우에만 AI 해석 대화에서 확인합니다. 계산 관련 현재 판정은 v2.2.2 문서를 기준으로 합니다.
...
이번 릴리즈에서는 UI만 바꾼 것이 아닙니다. 초기 v1 개발에 참고한 공개 배포 2019 Excel workbook, v1 GAS 계보, 현재 TypeScript 구현, 점수열과 구조요약 결과가 함께 공개된 외부 사례를 교차대조해 계산 로직의 일곱 경계 결함을 수정했습니다. 또한 Exner Comprehensive System 밖의 질문과 prompt injection을 차단하고, 한국어·일본어·영어 시나리오에서 코딩 도우미와 해석 도우미의 실제 GPT-5.5 production-parity 경로를 다시 평가했습니다.
...
## 계산 로직 독립 감사 (v2.2.1·v2.2.2에서 일부 판정 정정)
...
API key, `.env*`, Vercel local state, Neon connection string, raw prompt, raw model answer, chat message, response memo, Structural Summary private payload, patient data, 비공개 원문 provenance, 제3자 Excel 원본 파일, 운영 DB 정보, runtime log와 cache는 공개하지 않습니다.
