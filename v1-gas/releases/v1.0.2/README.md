...
> **v2.2.5 정정 안내:** 아래의 당시 패치노트는 능동형·수동형 표기가 없는 `M`을 `Ma`, `Mp`, `Ma-p`와 나란한 개별 반응 부호로 설명했습니다. 이후 초기 Excel 입력 목록, RorScore의 합계 방식, v1과 v2 코드를 다시 대조한 결과, 개별 운동 반응은 능동형 `a`, 수동형 `p`, 능동-수동형 `a-p` 중 하나로 구분해야 하고 `M`은 구조요약의 사람운동 합계 항목으로 남겨야 함을 확인했습니다. v2.2.5에서는 능동형·수동형 표기가 없는 `M`, `FM`, `m`을 입력 선택지에서 제거하고, 구조요약의 합계 항목은 유지합니다. 아래 본문은 v1.0.2 당시의 수정 기록을 보존하기 위해 그대로 둡니다.
