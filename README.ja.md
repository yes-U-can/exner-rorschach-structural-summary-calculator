...
構造一覧表の入力値、計算結果、AIチャットの内容、APIキーは訪問統計に含まれません。計算式、コーディング基準、出力値は変更していないため、既存のプロトコルを再計算する必要はありません。詳細は[v2.2.12パッチノート](./v2-nextjs/releases/v2.2.12/README.ja.md)をご確認ください。
