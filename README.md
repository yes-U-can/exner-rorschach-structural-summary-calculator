...
이 저장소의 설명문은 프로그래밍을 모르는 임상심리사도 계산기의 변화와 한계를 확인할 수 있도록 작성합니다. 계산 결과에 영향을 주는 변경은 영향을 받는 조건, 기존 결과를 다시 계산해야 하는지, 확인에 사용한 근거를 먼저 설명하고 개발 세부 사항은 뒤에 둡니다.

- 공개 릴리즈 노트와 CHANGELOG의 공식 기록은 한국어로 작성합니다.
- GitHub 방문자가 프로젝트를 빠르게 이해할 수 있도록 [English overview](./README.en.md)를 함께 둡니다.
- 웹앱 안의 사용자 경험은 한국어, 영어, 일본어, 스페인어, 포르투갈어 5개 언어를 지원합니다.
- 릴리즈 노트를 5개 언어로 매번 중복 작성하지는 않습니다. 여러 번역본의 내용이 서로 달라지는 문제를 막기 위해서입니다.
- 명령어, 파일 경로, API 이름, 모델명처럼 정확한 식별이 필요한 이름은 원문 표기를 유지하되, 처음 등장할 때 뜻을 설명합니다.
- `v2-nextjs/source/` 안의 개발 자료도 사람이 읽는 첫 설명은 쉬운 말로 작성하고, 재현에 필요한 명령어와 세부 수치는 별도의 기술 구역에 둡니다.
...
- [Next.js] 버전 2 최신 릴리즈: [v2-nextjs/releases/v2.2.4](./v2-nextjs/releases/v2.2.4/)
...
- **v2.1.7:** README, 변경 기록, 릴리즈 노트가 따라야 할 공개 문서 원칙을 정했습니다.
...
각 패치노트에는 해당 버전에서 실제로 완료한 범위를 적었습니다. 뒤이은 버전에서 새로 발견해 보완한 내용은 그다음 패치노트에 이어서 설명합니다.
...
후속 점검에서는 채점 화면에 들어올 때마다 새 데이터·샘플 데이터·저장 데이터 중 시작 방식을 다시 선택하도록 복원하고, 참조 문서의 코드형 핵심 구절을 라이트·다크 모드에서 더 잘 보이는 붉은색으로 표시했습니다. 코딩 도우미에서 이전 대화를 읽으려고 위로 이동했을 때 나타나는 아래쪽 화살표도 입력 영역 바로 위에 놓이도록 바로잡았습니다.
