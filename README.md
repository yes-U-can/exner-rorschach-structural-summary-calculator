...
v2.2.7은 채점표의 세 가지 미완성 입력이 계산으로 이어지지 않도록 막은 버그 패치입니다. 위치 선택지에서 단독 `S`를 제거해 흰 여백 반응을 항상 `WS`, `DS`, `DdS`로 기록하게 했고, 같은 운동 계열의 부호를 한 반응에 중복 입력할 수 없게 했으며, 형태질을 비워 둔 채 계산할 수 없게 했습니다. 모든 반응이 순수 형태(`F`)인 기록의 Lambda는 무한대 기호 대신 순수 F 개수로 보고합니다.
